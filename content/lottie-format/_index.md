+++
title = "About the Lottie Format"
description = "What is the Lottie animation format? Learn about the JSON-based vector animation standard created by Airbnb, its benefits, and how it works across platforms including JavaFX."
weight = 20
+++

Create, manage and implement lightweight animations across websites, apps, presentations, social and more with [LottieFiles](https://lottiefiles.com/).

## What is Lottie?

Lottie is a JSON-based animation file format that enables designers to ship animations on any platform as easily as shipping static assets. Created by Airbnb, Lottie has become the industry standard for vector animations on mobile and web.

## Why Lottie?

* **Small file sizes**: Vector-based animations are significantly smaller than traditional video or GIF files.
* **Scalable**: Works perfectly at any resolution without quality loss.
* **Cross-platform**: Runs on iOS, Android, Web, and now JavaFX with Lottie4J.
* **Designer-friendly**: Animations are exported directly from Adobe After Effects or can be [created with various tools](https://lottiefiles.com/integrations#create-animation).
* **Interactive**: Animations can be modified at runtime (colors, text, speed, etc.).

## Topics

{{% children depth="3" sort="date" order="desc" %}}
